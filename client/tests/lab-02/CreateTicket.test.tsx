...
describe("Create Ticket", () => {
  it("shows validation errors when required fields are invalid", async () => {
    ...
    /*
     * UI style assertion:
     * the Submit action uses Bootstrap button styling.
     */
    ...
    /*
     * UI style assertion:
     * validation feedback uses Bootstrap invalid-feedback styling.
     */
    expect(categoryError).toHaveClass(
      "invalid-feedback"
    );
    ...
    expect(
      screen.getByText(
        /summary must be between 5 and 120 characters/i
      )
    ).toBeInTheDocument();

    expect(
      screen.getByText(
        /description must be between 10 and 4000 characters/i
      )
    ).toBeInTheDocument();
...
it(
  "submits a valid ticket and shows the generated ticket number",
  async () => {
...
it(
  "prevents duplicate submission while ticket creation is in progress",
  async () => {
...
it(
  "shows a safe error and keeps form values when ticket creation fails",
  async () => {
